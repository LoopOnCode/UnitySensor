// Modified from https://github.com/rain931215/ESPHome-LD2410
#include "esphome.h"
...
public:
  ...
  Number *movingDistanceRange;
  Number *noneDuration;
  Number *sensitivityThreshold;
  int movingSensitivities[9] = {0};
  int stillSensitivities[9] = {0};

  long lastPeriodicMillis = millis();

  void setNumbers(Number *movingDistanceRange_, Number *noneDuration_, Number *sensitivityThreshold_)
  {
    movingDistanceRange = movingDistanceRange_;
    noneDuration = noneDuration_;
    sensitivityThreshold = sensitivityThreshold_;
  }
  ...
  void handleACKData(char *buffer, int len)
  {
    ...
    case 0x61: // Query parameters response
    {
      ...
      /*
        Moving distance range: 13th byte (buffer[12])
        Still distance range: 14th byte (buffer[13])
      */
      movingDistanceRange->publish_state(buffer[12] * 0.75);
      /*
      ...
      */
      ...
      sensitivityThreshold->publish_state(movingSensitivities[4]);
      /*
  ...
  void setMaxDistancesAndNoneDuration(int maxDistanceRange, int noneDuration)
  {
    char cmd[2] = {0x60, 0x00};
    char value[18] = {0x00, 0x00, lowByte(maxDistanceRange), highByte(maxDistanceRange), 0x00, 0x00, 0x01, 0x00, lowByte(maxDistanceRange), highByte(maxDistanceRange), 0x00, 0x00, 0x02, 0x00, lowByte(noneDuration), highByte(noneDuration), 0x00, 0x00};
